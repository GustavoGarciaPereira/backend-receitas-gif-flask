...
bioinformatica = Blueprint("contagem", __name__, url_prefix="/api/bio")
...
def contagem_bases():
    ...
    return jsonify([{"status": "ok"}, {"contagem": mapa_contagem}])
...
def rna_bases():
    ...
    mapa_contagem = {"A": "A", "C": "C", "T": "U", "G": "G"}

    saida = ""
    for letra in base_dna:
        saida += mapa_contagem[letra]

    return jsonify([{"status": "ok"}, {"base_rna": saida}])
...
def bases_inversa():
    ...
    mapa_contagem = {"A": "T", "C": "G", "T": "A", "G": "C"}

    saida = ""
    for letra in base_dna:
        saida += mapa_contagem[letra]

    return jsonify([{"status": "ok"}, {"base_inversa": saida}])
